---
name: diet-record
description: 飲食營養記錄技能。當使用者輸入文字或上傳食物／營養成份表圖片時觸發；自動保存食物外觀照片（營養成份表照片不存）、沿用歷史包裝標示資料、寫入 JSON、產生後續餐點建議、commit & push 並回報總結。
---
...
### Step 2：排除營養成份表照片並保存食物外觀照

不論是 GUI 對話附件或本機照片路徑，先逐張建立附件清單並分類。這裡的「排除／移除」是指不把該照片存進專案，也不寫進 daily JSON；不要刪除使用者上傳的原始附件。

- **食物外觀照片**：盤子、餐盒、食物本體，或可辨識實際食用品項的包裝正面 → 需保存到 `images/meals/`
- **營養成份表照片**：包裝背面的營養標示、超商標籤、成份表 → **只用來讀取數值，不存檔**
- **收據／價格照**：用來抓 `cost`，亦不存檔

附件篩選順序（必做）：

1. 逐張辨識本回合上傳圖片，標記為食物外觀照片、營養成份表照片或收據／價格照。
2. 從待保存清單排除營養成份表照片與收據／價格照；這些照片不得複製到 `images/meals/`。
3. 將待保存清單中的每張食物外觀照片存到 `images/meals/`。
4. 更新 daily JSON 時，讓對應食物的 `items[].image` 指向保存後的食物外觀照路徑。
5. 寫入後再次確認：daily JSON 不可引用營養成份表照片或收據／價格照。
...
- 只有食物照 → 寫入該檔路徑。
- 一張食物外觀照若涵蓋同餐多個品項，可讓對應的多個 items 共用同一路徑。
...
- 如本回合保存食物外觀照，確認至少一個對應 `items[].image` 寫入保存後路徑；不得誤填營養成份表照片路徑
- 重新計算該餐 subtotal
- 重新計算 dailyTotal（所有餐別加總）
- 重新計算 dailyCostTotal（所有 `meal.cost.amount` 加總；允許小數，例如分攤價格）

### Step 5：生成 AI 營養師建議與後續餐點組合

讀取 `data/goals.json` 作為唯一每日營養目標來源；不要在 skill 內硬編固定目標。比較 `dailyTotal` 與 goals 後，計算：

- `remainingCalories = goals.dailyCalories - dailyTotal.calories`
- `remainingProtein = goals.protein - dailyTotal.protein`
- `remainingFat = goals.fat - dailyTotal.fat`
- `remainingCarbs = goals.carbs - dailyTotal.carbs`

建議內容要求（繁體中文）：

1. 先寫 2-4 條營養狀態建議，包含各營養素達標狀況（✅ 達標 / ⚠️ 不足 / ❌ 超標）、具體調整食物、明日飲食方向。
2. 在同一個 `advice` 字串下方追加「🍱 接下來餐點組合建議」段落；`advice` 仍是單一字串，用 `\n` 分行，不新增 JSON 欄位。
3. 餐點推薦以每日營養目標補齊為標準，優先參考過去 365 天飲食紀錄，再使用台灣容易買到的外食或超商組合。

後續餐點推薦規則：

- **只記早餐後**：推薦午餐 + 晚餐，把剩餘熱量與碳水平均分配，蛋白質依不足量補足。
- **只記午餐且當日沒有早餐**：視為早餐漏吃，推薦晚餐 + 消夜/宵夜補齊；避免把缺口全部塞進晚餐，晚餐抓剩餘熱量約 60-75%，消夜抓約 25-40%，消夜以高蛋白、低脂、好消化為主。
- **已記早餐 + 午餐後**：推薦晚餐，必要時加一句份量調整，不主動推薦消夜。
- **已記到晚餐但仍明顯不足**：只有在熱量低於目標 15% 以上、蛋白質尚差 20g 以上，或碳水尚差 50g 以上且脂肪未超標時，才推薦點心或消夜。
- **熱量或脂肪已超標**：不要硬補消夜；改推薦減量、低油、少澱粉、避開炸物/起司/奶油醬，並把後續餐點份量降到維持飽足與蛋白質即可。
- 「宵夜」與既有 JSON meal key `消夜` 視為同一餐別；寫 JSON 仍使用 `消夜`。

歷史紀錄候選規則：

1. 掃描 `data/meals/**/*.json` 中日期距離今日 365 天內的紀錄。
2. 優先找餐別相符的 `meal.displayName` 或 `meal.items[]` 組合，且 `subtotal` 接近目前缺口與該餐分配目標。
3. 若 `remainingFat <= 0` 或脂肪接近上限，排除高脂組合（炸物、起司、奶油、肥肉、拉麵、重油便當）。
4. 若使用歷史餐點，文字可寫「參考 YYYY-MM-DD <餐別>」或「沿用常見紀錄」，並列出估算營養。
5. 歷史紀錄不足或不適合時，改用台灣常見可買到選項，例如：超商飯糰 + 茶葉蛋 + 高蛋白飲、健康便當、自助餐白飯半碗/一碗 + 雞胸/魚 + 燙青菜、Subway 6 吋雞肉、低油火鍋、無糖豆漿、地瓜、香蕉。

推薦段落格式：

```
🍱 接下來餐點組合建議
晚餐：自助餐白飯 1 碗 + 滷雞腿去皮 + 燙青菜 2 份（約 620 kcal / P 38g / F 14g / C 82g）
消夜：無糖豆漿 300ml + 茶葉蛋 1 顆 + 香蕉 1 根（約 255 kcal / P 16g / F 9g / C 27g）
份量調整：依每日營養目標尚缺 850 kcal、蛋白質 45g、碳水 110g；晚餐補主餐，消夜只補蛋白質與少量碳水，避免一次吃太撐。
```
...
```
🍱 接下來餐點組合建議
晚餐：低油魚肉或雞胸 1 份 + 燙青菜 2 份，白飯最多半碗（約 350 kcal / P 35g / F 8g / C 35g）
份量調整：今日熱量或脂肪已超標，不建議再補消夜；若餓，以無糖豆漿或茶葉蛋擇一即可。
```

將完整建議寫入 JSON 的 `advice` 欄位。
...
🍱 接下來餐點組合建議
晚餐：自助餐白飯 1 碗 + 滷雞腿去皮 + 燙青菜 2 份（約 620 kcal / P 38g / F 14g / C 82g）
份量調整：依每日營養目標補足剩餘熱量與蛋白質；若脂肪已接近上限，避開炸物、起司與奶油醬。
