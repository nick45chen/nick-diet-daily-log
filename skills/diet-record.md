...
1. **GUI 對話附件**：先檢查本回合訊息是否提供附件來源資訊。
   ...
   - 若平台只提供可視覺辨識的圖片內容，沒有任何本機路徑、檔案 handle 或可下載 URL，則只能用來分類與讀取營養資訊，**不能把原始圖片 bytes 寫入 repo**。若這張是食物照片，請先停止寫入 JSON / commit，要求使用者補一份原始檔案路徑；不要建立空白檔、截圖替代檔或假路徑。
...
- 有食物照但附件沒有可保存來源 → 先停止並要求補原始檔案路徑；不要把 `image` 寫成空字串後宣稱已保存。
