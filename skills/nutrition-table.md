# 台灣常見食物營養參考表

估算營養素時參考此表，以每份為單位。

**使用優先順序：**
1. 先到 `data/meals/**/*.json` 找同品項且 `note` 含「包裝標示」的最新一筆 → 沿用其數值，note 寫「包裝標示（沿用 YYYY-MM-DD <餐別>）」。
2. 本回合若使用者附上營養成份表照片，且數值較新或不同 → 改以照片為準，note 寫「包裝標示（本回合附件）」。
3. 上述都沒有 → 才用本表估算，且 `note` 註明「估算」或「依照片估算」。

| 食物 | 熱量 | 蛋白質 | 脂肪 | 碳水 |
|------|------|--------|------|------|
| 白飯 1碗 (200g) | 280 | 5 | 0.5 | 62 |
| 雞胸肉 100g | 110 | 23 | 1.3 | 0 |
| 水煮蛋 1顆 | 75 | 7 | 5 | 0.5 |
| 7-11 御飯糰 | 220 | 6 | 3 | 42 |
| 7-11 三明治 (雞肉) | 320 | 15 | 12 | 38 |
| 超商沙拉 | 120 | 8 | 4 | 14 |
| 便當 (雞腿) | 750 | 35 | 25 | 90 |
| 便當 (排骨) | 800 | 30 | 30 | 95 |
| 滷雞腿 1支 | 250 | 28 | 14 | 2 |
| 燙青菜 1份 | 30 | 2 | 0.5 | 5 |
| 豆漿 無糖 300ml | 90 | 9 | 4 | 3 |
| 牛奶 鮮乳 240ml | 150 | 8 | 8 | 12 |
| 高蛋白牛乳 | 150 | 20 | 4 | 10 |
| 乳清蛋白 1份 | 120 | 24 | 1.5 | 3 |
| 地瓜 中型 1條 | 130 | 1.5 | 0.2 | 31 |
| 香蕉 1根 | 90 | 1 | 0.3 | 23 |
| 蘋果 1顆 | 80 | 0.4 | 0.2 | 21 |
| 火鍋 (豬肉) | 700 | 40 | 25 | 70 |
| 火鍋 (羊肉) | 750 | 43 | 27 | 70 |
| 拉麵 1碗 | 550 | 20 | 15 | 75 |
| Subway 6吋 (雞肉) | 350 | 25 | 5 | 48 |
| 冰美式 中杯 | 5 | 0 | 0 | 1 |
| 拿鐵 中杯 (全脂) | 180 | 9 | 9 | 14 |
